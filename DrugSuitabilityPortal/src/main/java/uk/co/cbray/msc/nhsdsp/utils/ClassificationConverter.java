...
/**
 * Helper class for converting objects into the classification model.
 * 
 * @author Connor Bray
 */
public class ClassificationConverter {

	public static List<Instance> convert(List<Incident> list,
			DataAccessObject dao) {
		List<Instance> instances = new ArrayList<Instance>();

		List<Effect> effect = dao.executeNamedQuery("Effect.findAll",
				Effect.class);
		...
		for (Incident i : list) {
			...
			instance.setClassifier(findInList(classifiers, i.getEffect()
					.getName()));
			...
			for (PatientDetail detail : detailsSet) {
				...
				DiabetesFeature diabetes = new DiabetesFeature(
						detail.isDiabetes());
				EthnicityFeature ethnicity = new EthnicityFeature(
						detail.getEthnicity());
	...
	private static EffectClassifier findInList(List<EffectClassifier> effects,
			String name) {
